...
local TOOL_GUIDE = {
    ...
    "fresh diagnostics so you see breakage at once. Text that belongs where",
    "no symbol is - the top of a file, the end of one - goes in with",
    "insert_lines by line number, in several files at once if need be.",
    "The same change in many places is replace_pattern (regex or literal,",
    "kind=code to leave comments and string literals alone, dry_run first) -",
    "never a shell sed, which is outside the buffers, the undo ledger and the",
    "diagnostics. After moving code out of a file, unreferenced_symbols finds",
    "the definitions left behind: nothing else reports them, because an",
    "unreferenced function is not an error.",
