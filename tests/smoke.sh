...
#   headless:workspace, :index, :edit, :bytes, :verdict, :search, :files, :clients, :lifecycle
...
HEADLESS_GROUPS="workspace index edit indent ambiguity undo multifile bytes polish verdict search files tests clients lifecycle"
HEADLESS_ARGS=""
if [ $# -eq 0 ]; then
    SUITES="$ALL_SUITES"
    ASKED="$ALL_SUITES"
