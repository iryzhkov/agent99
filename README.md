...
One `agent99-bridge` serves every agent that talks to it, and one headless
Neovim serves every client that opened its root. Some of what the server
keeps is a property of the project and rightly shared — the buffers, the
language servers, the remembered check and test commands — and some of it is
a property of the caller, and used to be shared by accident:

- the **undo ledger**. It was one stack per root, ordered by wall clock
  across every caller, so `undo_edit()` with no arguments popped whatever the
  newest edit in the editor was. Two agents on one repository, one of them
  backing out a mistake, and the other's last edit left the disk with a
  success reply and nothing to say whose edit had gone.
- the **`check_project` and `run_tests` baselines**. A client that had never
  recorded one inherited another's, and "N new lines since the baseline"
  then described a change it had not made, measured from a tree state it had
  never seen.
- the **set of diagnostics a client has been shown**, which is what "new to
  this list" is measured against. One client's reply discharged another's
  disclosure: an agent was told "none new to this list" about warnings only
  the other agent had ever been shown.

All three are now keyed by `(root, client)`. `undo_edit` reaches only your
own edits and says how many steps the other clients have; each client
records its own baselines; and what is new to the list is new to *your*
list. `open_workspace` says whether the root was already open, how many
clients are using it, and which (`already_open`, `clients`, `shared`), and
`close_workspace` names the other clients whose workspace it is closing
(`was_shared`).

What counts as a client is what the transport can tell apart, and no more.
One MCP connection is one server process, so a second Claude Code session,
a second T3 thread and an editor-embedded bridge are each their own client.
The subagents of one session are not: they share that one connection, and
their `tools/call` requests carry a per-call `_meta.claudecode/toolUseId`
and a per-connection `progressToken` — nothing that groups one agent's calls
together. Such agents count as one client and share one ledger, which is
what `undo_edit`'s description says rather than promising isolation that is
not there. A harness that wants them separated has two ways to say so: send
`_meta["agent99/client"]` on each request, which is honoured per call, or
give each agent its own server process with `AGENT99_CLIENT_ID` set.
...
organizes its imports, and refuses to overwrite an existing file. The text
is written byte for byte, so a `text` that does not end in a newline makes a
file that does not either: it used to gain one, in no field of the reply,
while `verify=` was accepted and answered with nothing at all — which left
no field a caller could read the difference off. Undoing the create takes
the directories it made with it, and refuses when the file has been written
to since, rather than deleting content the tools never wrote.
...
### What the edit verdict covers

A language server publishes diagnostics for the documents something has
opened, so an edit that changes a symbol's shape breaks files nothing has
asked about. Before the edit, the tools that change a symbol or a file
(`replace_symbol_body`, `replace_symbol_lines`, `move_symbols`, `move_file`,
`delete_file`, `undo_edit`) work out the closure of that symbol over
`textDocument/references`, two hops deep — the files that use what the edited
file declares, and the files that use what those files declare — load them,
and wait for their servers along with the edited file's. The second hop is
what reaches a file whose error names neither the edited symbol nor the
edited file: a Go package that breaks because a variable it reads changed
type because the function behind it did.

The files are listed in the reply under `checked`, and the verdict names its
own scope: `no new errors or warnings in this file or in the 4 files that use
what it changed, listed under checked`. Where the closure cannot be worked
out — no server, a server that answers no reference request, a file with
nothing indexed as a declaration — the reply says so and says that importers
were not checked, rather than a bare "no new errors or warnings" that reads
as a statement about the project. The closure is bounded (24 files, 40
reference requests) and a cap that bites is reported too.

How long to hold on for the dependents is learned per workspace and server:
gopls answers for the edited file within a couple of hundred milliseconds and
...
| `skim` | structure of up to 20 files in one call: every function/class/method declaration line with line numbers, nested (treesitter, LSP-symbol fallback; C/C++ take the server's symbols first because macros confuse the grammar) — measures ~6-25% of the tokens of reading the same files. Markdown headings index like declarations, so a README's sections are name paths (`Install/Requirements`) for `find_symbol`, the section edits and grep's hit tags; a setext heading (`Title` over `====`) counts as one, and the spans come from the heading levels rather than from the grammar's own sections, which open for ATX headings only — without that, the heading above a setext one ran to the end of the document and a `replace_symbol_body` on an apparent four-line preamble would have overwritten it. Data files index by key the same way: a compose file's `services/api/environment`, a TOML `[server]` table, a JSON object's members, a Dockerfile's build stages (`FROM … AS runtime` up to the next `FROM`). A YAML list of mappings is descended into and each item is named by its own `name:` (a Kubernetes manifest is lists of containers, volumes and env vars, and that is most of what anyone edits in one) or by its position; a list of scalars is not, since its items are their own text and name nothing. An outline of a file that no longer parses carries `parse_error` and `partial`, so it is not mistaken for the outline of a healthy one. A file that declares nothing falls back to the server's symbols, where an array's elements arrive as one symbol each named after their own text: those are left out of the outline and counted, so a Lua table of package names outlines as its keys rather than as thirty strings. Build files too: a Makefile's targets and variables are symbols (`smoke` is the target with its recipe), and a shell script's are its functions and the variables it sets at the top — an assignment further in is a statement, not a declaration of the file. A QML file outlines as its object tree — the `Item`s, properties and signals, not only its JS functions — and a callback written as a single expression (`(store) => store.setPrompt`, which half a React file is made of) is not a declaration and is left out. A long file whose outline is trivial (one top-level key) is read as text rather than answered with the outline |
...
| `create_file`, `move_file`, `delete_file` | file lifecycle through the language servers (`workspace/*FileOperations`), so a refactor that adds, splits or renames a file does not have to leave the editor. `move_file` has the server rewrite the imports naming the old path before the move; `create_file` makes parent directories, formats and organizes imports, refuses to overwrite, writes the text byte for byte (a `text` with no final newline makes a file with none) and honours `verify=`; `delete_file` asks who used what the file declared, while the file is still there to ask from, loads those files and reports what broke in them — its verdict names them under `checked` rather than claiming the project. All three are undoable through `undo_edit` |
